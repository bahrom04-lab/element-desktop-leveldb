...
impl App {
    pub fn format_metadata(metadata: &Option<ElementMetadata>) -> String {
        match metadata {
            Some(meta) => {
    ...
    pub fn build_rooms(meta: &ElementMetadata) -> (Vec<(String, usize)>, Vec<(String, usize)>) {
        let rooms = meta
            .room_ids
            .iter()
            .enumerate()
